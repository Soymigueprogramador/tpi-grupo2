...
int posicionNumeroEnVector(int v[], int tam, int numero)
{
    ...
    {
        if (v[i] == numero)
            return i; // Si encuentra el número, devuelve la posición.
    }
...
void ordenarVector(int v[], int tam)
{
    ...
    for (i = 0; i < tam - 1; i++)
    {
        ...
        {
            if (v[j] < v[posmin])
                posmin = j; // Se busca un nuevo mínimo.
        }
...
int contarNumerosRepetidos(int v[], int numero, int tam)
{
    ...
    {
        if (v[i] == numero)
            cant++; // Si el número coincide, incrementa el contador.
    }
    return cant;
}
...
bool compararVectores(int v[], int v2[], int tam)
{
    ...
    {
        if (v2[i] != v[i])
            return false; // Si encuentra una diferencia, retorna false.
    }
