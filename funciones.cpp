...
void jugarPartida(string jugadorInicial, string jugadorOponente, int dadosInicial[], int dadosOponente[], int &cantDadosInicial, int &cantDadosOponente, int &puntosInicial, int &puntosOponente)
{
    ...
    for (int ronda = 1; ronda <= 3; ronda++) // entra en una ronda de 3
    {
        ...
        while (sumaParcial < numeroObjetivo) // Mientras la suma parcial sea menor al numero objetivo, se sigue eligiendo dados
        {
            ...
            {
                break;
            } // verifica siempre que si es igual, el usuario no tiene que seguir eligiendo dados, y se salta el while
        }
        ...
        else if (sumaSeleccionada == numeroObjetivo)
        { // Verifica si es igual al numeroObjetivo para saber si es tirada correcta o no
        ...
        else if (sumaSeleccionadaOponente == numeroObjetivoOponente)
        {
            cout << "Combinación elegida: ";
...
void estadisticasDelJuego( string jugador1, string jugador2,  int puntos1, int puntos2, int dados1, int dados2 ) {
    
    // Informacion que se mostrara en la consola
    ...
    // Condicional para validar que jugador gano o si tenemos empate 
    if ( puntos1 > puntos2 )
    {
        cout << " El ganador es el: " << jugador1 << endl;
